...
export default function PayrollReportsPage() {
  ...
  const { data, isLoading, error } = useGetAllPayroll();
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6 dark:bg-slate-800 dark:border-slate-700">
        ...
        {isLoading && (
          <p className="text-slate-600">Loading payroll reports...</p>
        )}
        {error && (
          <p className="text-red-600">Error loading payroll reports.</p>
        )}
        {!isLoading &&
          !error &&
          Array.isArray(filteredData) &&
          filteredData.length === 0 && (
            <p className="text-slate-600">No payroll reports found.</p>
          )}
        {!isLoading &&
          !error &&
          Array.isArray(filteredData) &&
